...
FileAttachment("group_3(分數版)@1.json").json()
)}
...
function _output_tree(d3,output,drag,url_list,invalidation)
{
  // 指定圖表的尺寸。
  const width = 800;
  const height = 550;
...
export default function define(runtime, observer) {
  ...
  const fileAttachments = new Map([
    ["group_3(分數版)@1.json", {url: new URL("../group_3(分數版).json", import.meta.url), mimeType: "application/json", toString}]
  ]);
  ...
  main.variable(observer("output_tree")).define("output_tree", ["d3","output","drag","url_list","invalidation"], _output_tree);
  return main;
}
