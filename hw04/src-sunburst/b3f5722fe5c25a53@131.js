...
md`# Data Visualization - hw04
### by 112C53023
參考 https://observablehq.com/d/c84affd948a1e733`
)}

function _artist(FileAttachment){return(
FileAttachment("artist_result.csv").csv()
)}

function _artist_result(__query,FileAttachment,invalidation){return(
__query(FileAttachment("artist_result.csv"),{from:{table:"artist_result"},sort:[],slice:{to:null,from:null},filter:[],select:{columns:null}},invalidation)
)}
...
function buildHierarchy(csv) {
    ...
    for (let i = 0; i < csv.length; i++) {
      ...
      const size = +csv[i][1];   //string to int number
      if (isNaN(size)) {
        continue;
      }
...
{
  var innerCircleAnswer = artist.map(row => row[innerCircleQuestion]);  //inner answers
  var outerCircleAnswer = artist.map(row => row[outerCircleQuestion]);  //outer answers
  ...
  var reformattedAnswers = combinedAnswers.map(item => {
    const [prefix, values] = item.split('-');   //from combinedAnswers
...
md`## 結論
### 從上圖中，我們可以看出：
- 此問卷的填寫者中，工作地點類型為工作室的占最多數，其次為替代空間。
- 大多數藝術工作者採取減少包裝的行動減少碳排放量
- 藝術園區的工作者傾向採取節約能源多於減少包裝以減少碳排放，其他則相反。`
)}
...
export default function define(runtime, observer) {
  ...
  const fileAttachments = new Map([
    ["artist_result.csv", {url: new URL("../artist_result.csv", import.meta.url), mimeType: "text/csv", toString}]
  ]);
  main.builtin("FileAttachment", runtime.fileAttachments(name => fileAttachments.get(name)));
  main.variable(observer()).define(["md"], _1);
  main.variable(observer("artist")).define("artist", ["FileAttachment"], _artist);
  main.variable(observer("artist_result")).define("artist_result", ["__query","FileAttachment","invalidation"], _artist_result);
